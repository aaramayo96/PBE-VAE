...
class PBEVAE:
    ...
    def predict(
        ...
        if sort_key not in {"mean_prob", "max_prob"}:
            raise ValueError("candidate_sort must be 'mean_prob' or 'max_prob'.")
        ...
        for key, refl_dat, refl_hdr in tqdm(files, desc="Running PBE-VAE inference"):
            ...
            known_rois_px = roi_centres_in_image(self.cfg, ds_hsi)
            known_roi_pixels_set = [(r, c) for (_, c, r) in known_rois_px]
            ...
            if np.any(binary):
                ...
                for blob_id in range(1, n_features + 1):
                    ...
                    is_known = any(
                        abs(row_c - kr) <= dedup_r and abs(col_c - kc) <= dedup_r
                        for (kr, kc) in known_roi_pixels_set
                    )
                    if is_known:
                        continue
            ...
            new_rois = sorted(new_rois, key=lambda r: r[sort_key], reverse=True)[:max_cand]
        ...
        if save:
            ...
            with open(txt_out, "w") as f:
                f.write("# Rank  Flight_Line  Lat  Lon  Pixels  Mean_Prob\n")
                for i, r in enumerate(
                    sorted(all_new_rois, key=lambda x: x[sort_key], reverse=True),
                    1,
                ):
                    f.write(
                        f"{i:4d}  {r['flight_line']:20s}  {r['lat']:.6f}  {r['lon']:.6f}  {r['n_pixels']:6d}  {r['mean_prob']:.4f}\n"
                    )
